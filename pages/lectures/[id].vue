<template>
    <div class="w-full" v-if="!loading">
        ...
        <div class="flex flex-wrap flex-col justify-center max-w-screen-xl m-auto md:px-4 md:py-8 border-b border-gray-400">
            <div class="flex flex-wrap flex-col w-full items-center justify-center">
                <div class="flex flex-wrap w-full items-start justify-center md:justify-between">
                    ...
                    <img :src="lecture.picture" :alt="lecture.title" class="md:rounded-xl w-full md:w-2/5  aspect-16/9 md:aspect-square object-cover">
                    <!--Other content-->
                    <div class="flex flex-wrap flex-col w-10/12 md:w-3/5 md:pl-8 md:pr-4 pl-0 pr-0 py-4">
                        <!--Name with the icon of user-->
                        <h2 class="info-label">
                            <span><font-awesome-icon :icon="['fas', 'book']" class="mr-2 text-lg"/></span>
                            Course
                        </h2>
                        <h3 class="info-content">
                            <span class="font-bold text-2xl">{{ lecture.title }}</span>
                        </h3>
                        ...
                        <p class="info-content">
                            {{ lecture.start.toLocaleString('en-Us') }} - {{ lecture.end }}
                        </p>
                        <h3 class="info-label">
                            <span><font-awesome-icon :icon="['fas', 'building']" class="mr-2 text-lg"/></span>
                            Location
                        </h3>
                        <p class="info-content">
                            {{ lecture.location }}
        ...
        <list-cards :items="speakers" title="Speakers" :isBackgroundDark="false" />
    </div>
</template>
...
async function getItem() {
    const { data, error } = await client.from('Lecture').select('*').eq('id', route.params.id).single();
    if (error) {
...
const lecture = await getItem();
console.log(lecture)

async function getSpeakers() {
    const { data, error } = await client.from('Teach').select('speaker_id (id, name, summary, picture)').eq('lecture_id', route.params.id);
    if (error) {
        console.error('Error fetching data:', error.message);
        return [];
    }
    ...
    // title -> speaker.name
    // subtitle -> speaker.summary
    // image_url -> speaker.picture
    // link -> speakers/speaker.id
    let result = []
    data.forEach((item) => {
        result.push({
            title: item.speaker_id.name,
            subtitle: item.speaker_id.summary,
            image_url: item.speaker_id.picture,
            link: '/speakers/' + item.speaker_id.id,
        })
    })
    return result;
}
...
function getBreadCrumb(route) {
    ...
    crumb[crumb.length - 1].name = lecture.title;
    return crumb;
}
